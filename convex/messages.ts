...
export const send = mutation({
    ...
    handler:async(ctx,args)=>{
    // Saving the user message with preserved new Lines
...
export const store = mutation({
    ...
    handler:async(ctx, args)=>{
        // Storing the messages with preserved newlines and html
        const messageId = await ctx.db.insert("messages",{
            chatId:args.chatId,
            content:args.content.replace(/\n/g,"\\n")
            // Dont escape HTML - we trust content since its generated in our system
